...
def test_create_collection_api(
...
def test_get_collections_api(
    ...
    response = client.get(
        f"/api/v1/collections?organization_id={test_organization.id}",
...
def test_update_collection_api(
    client, session, test_organization, test_collection, test_membership, auth_headers
):
    """Test updating a collection via API"""
    # Test: PUT /collections/{id} updates collection

    update_data = {"name": "Updated Collection", "status": "active"}
...
def test_delete_collection_api(
    client, session, test_organization, test_collection, test_membership, auth_headers
):
...
def test_collection_access_permissions(
    client, session, test_organization, test_collection, test_user, auth_headers
):
    ...
    memberships = (
        session.query(OrganizationMembership).filter_by(user_id=test_user.id).all()
    )
