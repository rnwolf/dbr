...
def test_role(session):
    ...
    if not role:
        ...
        role = Role(name=RoleName.PLANNER, description="Planner role for testing")
        session.add(role)
        session.commit()
    return role
...
def test_create_work_item_api(
    client, session, test_organization, test_collection, test_membership, auth_headers
):
    ...
    work_item_data = {
        ...
        "ccr_hours_required": {"development": 15.0, "testing": 5.0},
        "estimated_sales_price": 8000.0,
        "estimated_variable_cost": 2400.0,
    }

    response = client.post(
        "/api/v1/workitems", json=work_item_data, headers=auth_headers
    )
...
def test_create_work_item_validation(
    client, session, test_organization, test_membership, auth_headers
):
    ...
    response = client.post(
        "/api/v1/workitems", json=incomplete_data, headers=auth_headers
    )
    ...
    response = client.post(
        "/api/v1/workitems", json=invalid_status_data, headers=auth_headers
    )
    ...
    response = client.post(
        "/api/v1/workitems", json=invalid_priority_data, headers=auth_headers
    )
    assert response.status_code == 422


def test_work_item_crud_api(
    client, session, test_organization, test_work_items, test_membership, auth_headers
):
    ...
    response = client.get(
        f"/api/v1/workitems?organization_id={test_organization.id}",
    ...
    response = client.get(
        f"/api/v1/workitems?organization_id={test_organization.id}&status=Ready",
        headers=auth_headers,
    )
    assert response.status_code == 200
    ...
    response = client.get(
        f"/api/v1/workitems?organization_id={test_organization.id}&collection_id={collection_id}",
        headers=auth_headers,
    )
    assert response.status_code == 200
    ...
    response = client.delete(
        f"/api/v1/workitems/{work_item_to_delete}?organization_id={test_organization.id}",
        headers=auth_headers,
    )
...
def test_work_item_tasks_api(
    ...
    task_update = {"title": "Updated Task 1", "completed": True}
    ...
    updated_task = next(
        task for task in updated_work_item["tasks"] if task["id"] == task_id
    )
...
def test_work_item_error_handling(
    ...
    response = client.get(
        f"/api/v1/workitems/non-existent-id?organization_id={test_organization.id}",
        headers=auth_headers,
    )
    assert response.status_code == 404
    ...
    response = client.get(
        f"/api/v1/workitems?organization_id={fake_org_id}", headers=auth_headers
    )
...
def test_work_item_write_operations_require_authentication(
    client, session, test_organization, test_work_items
):
    ...
    assert response.status_code in [401, 403], (
        "Create work item should require authentication"
    )
    ...
    assert response.status_code in [401, 403], (
        "Update work item should require authentication"
    )
    ...
    assert response.status_code in [401, 403], (
        "Delete work item should require authentication"
    )
    ...
    assert response.status_code in [401, 403], (
        "Update task should require authentication"
    )


def test_work_item_filtering_and_sorting(
    client, session, test_organization, test_work_items, test_membership, auth_headers
):
