...
def test_schedule_creation():
    ...
    from dbr.models.work_item_dependency import WorkItemDependency  # Import to ensure table is created
    from dbr.models.ccr_user_association import CCRUserAssociation  # Import to ensure table is created
    from dbr.models.organization_membership import OrganizationMembership  # Import to ensure table is created
...
def test_schedule_capacity_validation():
    ...
    try:
        ...
        with SessionLocal() as session:
            ...
            over_capacity_items = ready_items + [ready_items[0]]  # 3 items * 10 hours = 30 hours (at capacity)
            ...
            with pytest.raises(ScheduleValidationError, match="Schedule requires .* hours but CCR capacity is"):
                over_capacity_schedule.validate_capacity(session)
            
            # Test: Multiple work items fit within capacity
            valid_item_ids = [item.id for item in ready_items]  # 2 items * 10 hours = 20 hours
