...
class LoginDialog(ctk.CTkToplevel):
    ...
    def _create_widgets(self) -> None:
        ...
        self.subtitle_label = ctk.CTkLabel(
            self, text="Please log in to continue", font=ctk.CTkFont(size=14)
        )
        ...
        self.username_entry = ctk.CTkEntry(
            self.form_frame, placeholder_text="Enter username", width=300
        )

        self.password_label = ctk.CTkLabel(self.form_frame, text="Password:")
        self.password_entry = ctk.CTkEntry(
            self.form_frame, placeholder_text="Enter password", show="*", width=300
        )

        # Buttons
        self.button_frame = ctk.CTkFrame(self.form_frame, fg_color="transparent")
        self.login_button = ctk.CTkButton(
            self.button_frame, text="Login", command=self._on_login_click, width=140
        )
        ...
        self.status_label = ctk.CTkLabel(
            self, text="", font=ctk.CTkFont(size=12), text_color="red"
        )
        ...
        try:
            if self.dbr_service.health_check():
                self.connection_label.configure(
                    text=f"✅ Connected to {self.backend_url}", text_color="green"
                )
            else:
                self.connection_label.configure(
                    text=f"❌ Cannot connect to {self.backend_url}", text_color="red"
                )
        except Exception as e:
            self.connection_label.configure(
                text=f"❌ Connection error: {str(e)}", text_color="red"
            )
...
class UserContextWidget(ctk.CTkFrame):
    ...
    def __init__(
        self, parent, user_info: Dict[str, Any], user_role: str, organization_name: str
    ):
        super().__init__(parent)
    ...
    def _create_widgets(self) -> None:
        ...
        # User info
        display_name = self.user_info.get(
            "display_name", self.user_info.get("username", "Unknown")
        )

        self.user_label = ctk.CTkLabel(
            self, text=f"👤 {display_name}", font=ctk.CTkFont(size=12, weight="bold")
        )

        self.role_label = ctk.CTkLabel(
            self, text=f"🔑 {self.user_role or 'No Role'}", font=ctk.CTkFont(size=11)
        )

        self.org_label = ctk.CTkLabel(
            self, text=f"🏢 {self.organization_name}", font=ctk.CTkFont(size=11)
        )
    ...
    def authenticate(self) -> Optional[DBRService]:
        ...
        try:
            ...
            messagebox.showerror(
                "Authentication Error", f"Authentication failed: {str(e)}"
            )
            return None
