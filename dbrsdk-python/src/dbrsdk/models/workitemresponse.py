"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .taskresponse import TaskResponse, TaskResponseTypedDict
from dbrsdk.types import BaseModel, Nullable, UNSET_SENTINEL
from pydantic import model_serializer
from typing import Dict, List
from typing_extensions import TypedDict


class WorkItemResponseTypedDict(TypedDict):
    id: str
    organization_id: str
    collection_id: Nullable[str]
    title: str
    description: Nullable[str]
    status: str
    priority: str
    estimated_total_hours: float
    ccr_hours_required: Nullable[Dict[str, float]]
    estimated_sales_price: Nullable[float]
    estimated_variable_cost: Nullable[float]
    throughput: float
    tasks: List[TaskResponseTypedDict]
    progress_percentage: float
    responsible_user_id: Nullable[str]
    url: Nullable[str]
    created_date: str
    updated_date: str


class WorkItemResponse(BaseModel):
    id: str

    organization_id: str

    collection_id: Nullable[str]

    title: str

    description: Nullable[str]

    status: str

    priority: str

    estimated_total_hours: float

    ccr_hours_required: Nullable[Dict[str, float]]

    estimated_sales_price: Nullable[float]

    estimated_variable_cost: Nullable[float]

    throughput: float

    tasks: List[TaskResponse]

    progress_percentage: float

    responsible_user_id: Nullable[str]

    url: Nullable[str]

    created_date: str

    updated_date: str

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = []
        nullable_fields = [
            "collection_id",
            "description",
            "ccr_hours_required",
            "estimated_sales_price",
            "estimated_variable_cost",
            "responsible_user_id",
            "url",
        ]
        null_default_fields = []

        serialized = handler(self)

        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)
            serialized.pop(k, None)

            optional_nullable = k in optional_fields and k in nullable_fields
            is_set = (
                self.__pydantic_fields_set__.intersection({n})
                or k in null_default_fields
            )  # pylint: disable=no-member

            if val is not None and val != UNSET_SENTINEL:
                m[k] = val
            elif val != UNSET_SENTINEL and (
                not k in optional_fields or (optional_nullable and is_set)
            ):
                m[k] = val

        return m
